...
export class SaveCurrentRecordsCommand<T extends BaseEntity> implements ICommand
{
    constructor(private colletion : T[], private facade : IUseCaseFacade<T>)
    {}
    async Execute(): Promise<void> {
        this.colletion.forEach(entity =>{
            this.facade.Update(entity.Id as string, entity)
            .subscribe({next:(response:FireBaseResponse) =>{
                if(response.status)
                {
